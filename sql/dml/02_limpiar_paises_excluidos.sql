...
-- =============================================================================
-- Tras restringir el alcance del proyecto a Uruguay + Brasil + Argentina,
-- este script elimina de la BD los registros cargados de Bolivia, Paraguay,
-- Chile y Perú que quedaron de la versión previa (que abarcaba 7 países).
--
...
UNION ALL
SELECT 'puntos a borrar (no URY/BRA/ARG)', COUNT(*)
FROM puntos_monitoreo WHERE pais NOT IN ('URY','BRA','ARG')
UNION ALL
SELECT 'focos_calor (total)', COUNT(*) FROM focos_calor
UNION ALL
SELECT 'focos a borrar (no URY/BRA/ARG o NULL)', COUNT(*)
FROM focos_calor WHERE pais NOT IN ('URY','BRA','ARG') OR pais IS NULL
UNION ALL
...
WHERE id_punto IN (
    SELECT id FROM puntos_monitoreo WHERE pais NOT IN ('URY','BRA','ARG')
);
...
WHERE pais NOT IN ('URY','BRA','ARG') OR pais IS NULL;

-- ── 3. Dimensiones ───────────────────────────────────────────────────────

DELETE FROM puntos_monitoreo
WHERE pais NOT IN ('URY','BRA','ARG');

DELETE FROM paises_referencia
WHERE codigo_iso3 NOT IN ('URY','BRA','ARG');

-- Asegurar que los 3 países objetivo están presentes (idempotente).
